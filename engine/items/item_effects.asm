...
PokeBallEffect:
; BUGFIX: The Dude's catching tutorial may crash if his Poké Ball can't be used (see docs/bugs_and_glitches.md)
	ld a, [wBattleMode]
...
.room_in_party
; BUGFIX: Using a Park Ball in non-Contest battles has a corrupt animation (see docs/bugs_and_glitches.md)
	xor a
	ld [wWildMon], a
	ld a, [wBattleType]
...
.okay_1
	...
	ld a, 1
.statuscheck
; BUGFIX: BRN/PSN/PAR do not affect catch rate (see docs/bugs_and_glitches.md)
	ld b, a
...
; BUGFIX: HELD_CATCH_CHANCE has no effect (see docs/bugs_and_glitches.md)
	ld d, a
	push de
	ld a, [wBattleMonItem]
	ld b, a
...
; BUGFIX: Catching a Transformed Pokémon always catches a Ditto (see docs/bugs_and_glitches.md)
...
; BUGFIX: Heavy Ball uses wrong weight value for three Pokémon (see docs/bugs_and_glitches.md)
	push hl
	push de
	ld a, [wEnemyMonSpecies]
	dec a
...
; BUGFIX: Moon Ball does not boost catch rate (see docs/bugs_and_glitches.md)
	push bc
	ld a, BANK("Evolutions and Attacks")
	call GetFarByte
	cp MOON_STONE
...
; BUGFIX: Love Ball boosts catch rate for the wrong gender (see docs/bugs_and_glitches.md)
	ld a, d
	pop de
	cp d
	pop bc
...
.loop
; BUGFIX: Fast Ball only boosts catch rate for three Pokémon (see docs/bugs_and_glitches.md)
	ld a, BANK(FleeMons)
	call GetFarByte
